...
class block_detector:
    ...
    def create_pose(self, x, y, angle, dist):
        ...
        p.pose.position.x = result[0]#result[2]
        p.pose.position.y = result[1]#-result[0]
        p.pose.position.z = result[2]#-result[1]
    ...
    def cam2rob_transform(self, pose, frame):
        ...
        p_in_base.pose.orientation.z = -(-box_angle[2]+90) #= -box_angle[2]+90
    ...
    def process_img(self, image, depth_image):
        ...
        cv2.namedWindow("col_filtered", cv2.WINDOW_NORMAL) 
            ...
            try:
                ...
                if area > 90:
                    ...
                    if shape is not None:
                        ...
                        p = self.cam2rob_transform(p, "/camera_color_optical_frame")
...
def realsense_run():
    ...
    detector = block_detector(frame_id)
