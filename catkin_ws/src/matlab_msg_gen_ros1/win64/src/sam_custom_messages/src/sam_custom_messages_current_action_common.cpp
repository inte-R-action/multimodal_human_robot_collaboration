...
// Common copy functions for sam_custom_messages/current_action
...
#endif //_MSC_VER
#include "ros/ros.h"
#include "sam_custom_messages/current_action.h"
...
namespace current_action {
  void copy_from_arr_std_msgs_Header(std_msgs::Header& val, const matlab::data::StructArray& arr);
  MDArray_T get_arr_std_msgs_Header(MDFactory_T& factory, const sam_custom_messages::current_action::_header_type& val);
  ...
  //----------------------------------------------------------------------------
  SAM_CUSTOM_MESSAGES_EXPORT void copy_from_arr(boost::shared_ptr<sam_custom_messages::current_action>& msg, const matlab::data::StructArray arr) {
    try {
        //header
    ...
    try {
        //action_probs
        const matlab::data::TypedArray<double> action_probs_arr = arr[0]["action_probs"];
        size_t nelem = action_probs_arr.getNumberOfElements();
        	msg->action_probs.resize(nelem);
        	std::copy(action_probs_arr.begin(), action_probs_arr.begin()+nelem, msg->action_probs.begin());
    } catch (matlab::data::InvalidFieldNameException&) {
        throw std::invalid_argument("Field 'action_probs' is missing.");
    } catch (matlab::data::TypeMismatchException&) {
        throw std::invalid_argument("Field 'action_probs' is wrong type; expected a double.");
    }
  }
  //----------------------------------------------------------------------------
  SAM_CUSTOM_MESSAGES_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const sam_custom_messages::current_action>& msg) {
    auto outArray = factory.createStructArray({1,1},{"header","user_id","action_probs"});
    // header
    outArray[0]["header"] = get_arr_std_msgs_Header(factory, msg->header);
    // user_id
    outArray[0]["user_id"] = factory.createScalar(msg->user_id);
    // action_probs
    outArray[0]["action_probs"] = factory.createArray<sam_custom_messages::current_action::_action_probs_type::const_iterator, double>({1, msg->action_probs.size()}, msg->action_probs.begin(), msg->action_probs.end());
