...
// Common copy functions for sam_custom_messages/user_prediction
...
namespace user_prediction {
  ...
  MDArray_T get_arr_std_msgs_Header(MDFactory_T& factory, const sam_custom_messages::user_prediction::_header_type& val);
  ...
  //----------------------------------------------------------------------------
  SAM_CUSTOM_MESSAGES_EXPORT void copy_from_arr(boost::shared_ptr<sam_custom_messages::user_prediction>& msg, const matlab::data::StructArray arr) {
    try {
        //header
    ...
    try {
        //predictions
        const matlab::data::TypedArray<double> predictions_arr = arr[0]["predictions"];
        size_t nelem = predictions_arr.getNumberOfElements();
        	msg->predictions.resize(nelem);
        	std::copy(predictions_arr.begin(), predictions_arr.begin()+nelem, msg->predictions.begin());
    } catch (matlab::data::InvalidFieldNameException&) {
        throw std::invalid_argument("Field 'predictions' is missing.");
    } catch (matlab::data::TypeMismatchException&) {
        throw std::invalid_argument("Field 'predictions' is wrong type; expected a double.");
    }
  }
  //----------------------------------------------------------------------------
  SAM_CUSTOM_MESSAGES_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const sam_custom_messages::user_prediction>& msg) {
    auto outArray = factory.createStructArray({1,1},{"header","user_id","predictions"});
    // header
    outArray[0]["header"] = get_arr_std_msgs_Header(factory, msg->header);
    // user_id
    outArray[0]["user_id"] = factory.createScalar(msg->user_id);
    // predictions
    outArray[0]["predictions"] = factory.createArray<sam_custom_messages::user_prediction::_predictions_type::const_iterator, double>({1, msg->predictions.size()}, msg->predictions.begin(), msg->predictions.end());
