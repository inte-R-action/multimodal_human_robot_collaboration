...
class User:
    ...
    def collate_har_seq(self):
        ...
        # Group predictions of same type together
        for a in range(len(self._har_state_hist)):
            ...
            if action == self._har_state_hist[a][-1, 0]:
                self._har_state_hist[a] = np.vstack((self._har_state_hist[a], [action, prob, self._har_pred_hist[-1, -1]]))
